...
RSpec.describe 'Items API', type: :request do
  # Αρχικοποίηση δεδομένων για αυθεντικοποίηση και scoping
  let(:user) { create(:user) }
  # Το todo πρέπει να ανήκει στον user για να έχει πρόσβαση ο controller
  let!(:todo) { create(:todo, created_by: user.id) }
  let!(:items) { create_list(:item, 20, todo_id: todo.id) }
  let(:todo_id) { todo.id }
  let(:id) { items.first.id }
  # Headers με το έγκυρο JWT token
  let(:headers) { valid_headers }

  # Test suite για GET /todos/:todo_id/items
  ...
  # Test suite για GET /todos/:todo_id/items/:id
  ...
  # Test suite για POST /todos/:todo_id/items
  ...
  # Test suite για PUT /todos/:todo_id/items/:id
  ...
  # Test suite για DELETE /todos/:todo_id/items/:id
